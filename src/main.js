...
const SCENE_INDICES = {
FRONT_OF_DOOR_PIVOT: 12, // Scene 12 (WP13) - lock scene (no zombies, just lock)
WAREHOUSE_INTERIOR: 15, // Scene 15 - first interior scene (after 14 exterior scenes: 0-13 + lock/post-lock)
WAREHOUSE_INTERIOR_FINAL: 16, // Scene 16 - final interior scene with boss fight
INTERIOR_START: 15 // First interior scene index
};
...
// Only enable free look for shooting scenes
if (!currentCameraScene || currentCameraScene.type !== 'shooting') {
mouseLookManager.lock();
mouseLookManager.disable();

// Still set camera lookAt for travel scenes (no free look)
...
console.log(`Camera lookAt: { x: ${currentCameraScene.lookAt.x.toFixed(2)}, y: ${currentCameraScene.lookAt.y.toFixed(2)}, z: ${currentCameraScene.lookAt.z.toFixed(2)} }`);
}
...
console.log(`Rail path completed - Scene ${sceneIndex}: ${sceneDefinition.name}`);

// Update current scene - CRITICAL: This must happen BEFORE any scene setup
gameData.currentScene = sceneIndex;
currentCameraScene = sceneDefinition;

console.log(`Scene ${sceneIndex} position: { x: ${currentCameraScene.position.x}, y: ${currentCameraScene.position.y}, z: ${currentCameraScene.position.z} }`);
...
// Load lock for Scene 12 (Final Exterior Encounter - WP13)
...
// Load lock at specified position - user requested coordinates for visibility
const lockPosition = new THREE.Vector3(-9.47, 1.29, 6.22); // Lock position for visibility
...
// Dispose lock if we're not on Scene 12 (lock scene)
if (lockManager) {
lockManager.dispose();
setLockManager(null);
}
...
// For interior scenes, check scene type before enabling free aim
if (sceneIndex >= SCENE_INDICES.INTERIOR_START) {
// Only enable free aim for shooting scenes
if (currentCameraScene && currentCameraScene.type === 'shooting') {
...
console.log(`Fallback: Spawning zombies for Scene ${gameData.currentScene}: ${currentScene.name}`);
...
console.log(`Scene ${gameData.currentScene} cleared - transitioning to next scene`);
...
console.log('\n═══════════════════════════════════════════════════════');
console.log(`Marked position: { x: ${camera.position.x.toFixed(2)}, y: ${camera.position.y.toFixed(2)}, z: ${camera.position.z.toFixed(2)} }`);
console.log('═══════════════════════════════════════════════════════\n');
...
console.warn('Factory scene load failed, creating fallback ground');
const groundGeo = new THREE.PlaneGeometry(200, 200);
const groundMat = new THREE.MeshStandardMaterial({ color: 0x2a2a2a });
const fallbackGround = new THREE.Mesh(groundGeo, groundMat);
fallbackGround.name = 'ground';
...
console.warn('Factory scene loading timeout, preparing scene anyway');
currentCameraScene = CAMERA_SCENES[0];
if (!threeRenderer.getGround()) {
