...
/**
* LockManager
* Manages the lock on the warehouse door in Scene 12 (Final Exterior Encounter - WP13)
...
console.warn('Lock already loaded, skipping duplicate load');
return false;
}
...
// Position: move right (+X) and down (-Y) from base position for proper alignment
this.lockGroup.position.set(position.x + 0.5, position.y - 0.3, position.z);
...
// Ensure lock group and all children are visible
this.lockGroup.visible = true;
this.lockModel.visible = true;

// Double-check all meshes are visible
this.lockModel.traverse((child) => {
if (child.isMesh) {
child.visible = true;
if (child.material) {
...
// Log detailed lock info for debugging
console.log('Lock loaded and positioned at:', position);
console.log(`Lock group position: (${this.lockGroup.position.x.toFixed(2)}, ${this.lockGroup.position.y.toFixed(2)}, ${this.lockGroup.position.z.toFixed(2)})`);
console.log(`Lock group visible: ${this.lockGroup.visible}`);
console.log(`Lock model visible: ${this.lockModel.visible}`);
console.log(`Lock group in scene: ${this.scene.children.includes(this.lockGroup)}`);
console.log(`Lock isActive: ${this.isActive()}`);
