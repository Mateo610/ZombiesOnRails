...
export class SceneSetupManager {
    ...
    setupWarehouseVisibility(setGround = true) {
        if (!this.sceneLoader.warehouseModel) {
            console.warn('⚠️ Warehouse model not loaded yet');
            return;
        }
    ...
    setupInteriorFloor() {
        ...
        if (floorCandidates.length > 0) {
            ...
            console.log('⚠️ No floor found in model, created fallback floor');
        }
    }
    ...
    spawnSceneZombies(gameData) {
        ...
        if (gameData.currentScene >= this.SCENE_INDICES.INTERIOR_START && 
            ...
            console.log(`⏭️ Skipping zombie spawn for interior scene ${gameData.currentScene + 1}`);
            return;
        }
        
        const currentScene = this.CAMERA_SCENES[gameData.currentScene];
        if (!currentScene) {
            console.warn(`⚠️ No scene config found for scene index ${gameData.currentScene}`);
            return;
        }
        
        if (!currentScene.spawnPoints || currentScene.spawnPoints.length === 0) {
            console.log(`ℹ️ No spawn points defined for Scene ${gameData.currentScene + 1}: ${currentScene.name}`);
            return;
        }
        
        console.log(`🎬 Spawning ${currentScene.spawnPoints.length} zombies for Scene ${gameData.currentScene + 1}: ${currentScene.name}`);
