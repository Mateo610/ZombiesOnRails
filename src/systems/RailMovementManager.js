...
this.startLookAt = null; // Starting lookAt direction for smooth interpolation
this.startLookAtDirection = null; // Starting direction vector for interpolation
...
console.warn('Failed to play footstep sound:', err);
});
} catch (error) {
...
console.warn('Error stopping footstep sound:', error);
}
}
}
...
console.warn('No rail paths defined');
return;
}
...
// Handle same-position paths (pause/dwell at waypoint)
if (this._isSamePositionPath) {
...
// Calculate smooth look-at direction with interpolation
...
// Smoothly interpolate between starting direction and target direction
// Use spherical linear interpolation (SLERP) for smooth rotation
const startDir = this.startLookAtDirection || new THREE.Vector3(0, 0, -1);
...
// If angle is very small, no interpolation needed
let interpolatedDirection;
if (angle < 0.001) {
interpolatedDirection = endDir.clone();
} else {
// Use SLERP for smooth rotation
const t = easedProgress; // Use eased progress for smooth acceleration/deceleration
const sinAngle = Math.sin(angle);
const startWeight = Math.sin(angle * (1 - t)) / sinAngle;
const endWeight = Math.sin(angle * t) / sinAngle;

interpolatedDirection = new THREE.Vector3()
.addScaledVector(startDir, startWeight)
.addScaledVector(endDir, endWeight)
.normalize();
}

// Calculate final lookAt point
const lookAt = new THREE.Vector3()
.copy(this.camera.position)
.addScaledVector(interpolatedDirection, this.lookAtDistance);

// Keep lookAt at appropriate height (use target's Y if it's higher)
lookAt.y = Math.max(this.camera.position.y, target.y) + 0.5;

// Set the base lookAt with smooth interpolation
...
// CRITICAL: Skip rail movement for transition from Scene 14 (index 14) to Scene 15 (interior)
// This transition should use fade-to-black and direct camera jump instead
// Scene 12 (lock) -> Scene 13 (shooting) -> Scene 14 (travel) -> Scene 15 (interior via fade-to-black)
if (this.gameData.currentScene === 14) {
console.log('Rail movement: Skipping rail movement for Scene 14 to Scene 15 transition (uses fade-to-black to interior)');
return false;
}
...
console.log('Rail movement: Game not in valid state for rail movement. Current:', this.gameData.currentState);
return false;
}
...
console.log('Rail movement: No more paths available');
...
console.warn(`Failed to preload models for Scene ${nextSceneIndex + 1}:`, error);
});
}
}
...
console.log(`Ending at: { x: ${endPos.x.toFixed(2)}, y: ${endPos.y.toFixed(2)}, z: ${endPos.z.toFixed(2)} }`);
console.log(`Distance: ${distance.toFixed(3)}`);
console.log(`Duration: ${this.currentPath.duration || this.duration}ms`);

// Special case: If start and end positions are the same (same-position path for pause/dwell)
// Still respect the duration but don't move the camera, only rotate lookAt
this._isSamePositionPath = distance < 0.001;
if (this._isSamePositionPath) {
console.log('Same-position path detected - camera will pause and only rotate lookAt');
}
...
try {
// Use curve type from path config if specified, otherwise default to 'centripetal'
// 'chordal' creates smoother, faster transitions through waypoints without slowing down
const curveType = (this.currentPath && this.currentPath.curveType) || 'centripetal';
this.splineCurve = new THREE.CatmullRomCurve3(points, false, curveType);
...
// Store starting lookAt direction for smooth interpolation
...
this.startLookAtDirection = currentLookDirection.clone();

// Calculate starting lookAt point (where camera is currently looking)
this.startLookAt = new THREE.Vector3()
.copy(this.camera.position)
.addScaledVector(currentLookDirection, this.lookAtDistance);
...
console.log('▶ Rail movement started - using spline interpolation');
console.log(`Start time: ${this.startTime}, Duration: ${this.duration}ms`);
...
console.log(`▶ Rail movement to scene started - duration: ${this.duration}ms`);
...
console.warn(`[forceCameraUpdate] Camera position was overridden! Distance: ${distance.toFixed(3)}m`);
