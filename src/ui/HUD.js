...
<span id="scene-number" style="color: #ffffff; font-weight: bold;">0</span>/
<span id="total-scenes" style="color: #cccccc;">16</span>
<span style="font-size: 10px; color: #666666; margin-left: 5px;">(0-indexed)</span>
</div>
...
console.error('startRailMovement function not available');
}
});
}
}
...
console.warn('Music is disabled, enabling it first');
...
indicator.title = `Scene ${i} (Index ${i}) - Completed`;
indicator.innerHTML = '✓';
...
indicator.title = `Scene ${i} (Index ${i}) - In Progress (${zombiesKilled}/${totalZombies})`;
} else {
// Upcoming scene - gray
indicator.style.background = 'rgba(153, 153, 153, 0.3)';
indicator.style.border = '1px solid rgba(153, 153, 153, 0.5)';
indicator.title = `Scene ${i} (Index ${i}) - Upcoming`;
}
...
// Scene - Display actual 0-indexed scene number to match code
document.getElementById('scene-number').textContent = gameData.currentScene;
// Total scenes should be CAMERA_SCENES.length, but use gameData.totalScenes as fallback
...
console.log('Game already started, ignoring confirm button click');
return;
}
